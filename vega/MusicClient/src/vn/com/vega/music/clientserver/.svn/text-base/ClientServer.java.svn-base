...
public class ClientServer {
	...
	private static final String CLIENT_PROFILE = "Android";
	private static final String SECRET_KEY = "pOwErOvErwhElmIng";
	private static final String CLIENT_VERSION = "20120403";
	...
	public boolean isCurrentServerIsPreferedFor3G() {
		if ((mCurrentServerIndex < 0)
				|| (mCurrentServerIndex >= mServerAddresses.size())) {
			selectNextServer();
		}
		if ((mCurrentServerIndex >= 0)
				&& (mCurrentServerIndex < mServerAddresses.size())) {
	...
	private boolean selectNextServer() {
		...
		if (network == NetworkUtility.CONNECTION_TYPE_3G) {
			for (int i = 0; i < mServerAddresses.size(); i++) {
				...
				if (!server.mTried
						&& (server.mNetworkPrefer == ServerAddress.PREF_3G)) {
		...
		for (int i = 0; i < mServerAddresses.size(); i++) {
			ServerAddress server = mServerAddresses.get(i);
			if (!server.mTried
					&& (server.mNetworkPrefer == ServerAddress.PREF_NONE)) {
				this.URL_API_ROOT = server.mServerAddress;
				mCurrentServerIndex = i;
				return true;
			}
	...
	protected String getUrlAuthLogin(String user, String pass) {
		checkSelectServer();
		return URL_API_ROOT + "/account/login/?username=" + user + "&password="
				+ pass;
	}

	protected String getUrlAuthValidate(String hash) {
		checkSelectServer();
		return URL_API_ROOT + "/auth/validate/?hash=" + hash + "&client="
				+ URLEncoder.encode(CLIENT_PROFILE) + "&version="
	...
	protected String getUrlAuthSubscribe(String packageID, String phoneNumber) {
		checkSelectServer();
		return URL_API_ROOT + "/account/register?packageId=" + packageID
	...
	protected String getUrlSetNotificationConfig(int param1, int param2,
			int param3, int param4, int param5) {
		checkSelectServer();
		return URL_API_ROOT
				+ "/account/updatenotificationconfig?email_on_new_playlist="
				+ param1 + "&email_on_like_playlist=" + param2
				+ "&email_on_service_news=" + param3 + "&sms_on_shared_to_me="
				+ param4 + "&sms_on_artist_updated=" + param5;
	}
	...
	protected String getUrlShareSong(int songId) {
		checkSelectServer();
		return URL_API_ROOT + "/songs/" + songId + "/getsharemessage";
	}

	/**
	 * Share song and playlist
	 */

	protected String GetUrlShareSong(int songId, int FriendId) {
		checkSelectServer();
		return URL_API_ROOT + "/songs/" + songId + "/share/" + FriendId;
	}

	protected String GetUrlShareSong(int songId, String friendList) {
		checkSelectServer();
		return URL_API_ROOT + "/songs/" + songId + "/share/" + friendList;
	}

	protected String GetUrlSharePlaylist(int playlistId, int friendId) {
		checkSelectServer();
		return URL_API_ROOT + "/playlist/" + playlistId + "/share/" + friendId;
	}

	protected String GetUrlSharePlaylist(int playlistId, String friendList) {
		checkSelectServer();
	...
	protected String getUrlSharePlaylist(int playlistId) {
		checkSelectServer();
		return URL_API_ROOT + "/playlist/" + playlistId + "/getsharemessage";
	}
	...
	protected String getUrlAuthUpdateAccount(String name, String phone,
			String email, String dob, int gender, String address) {
		checkSelectServer();
		String url = URL_API_ROOT + "/account/updateinfo?full_name="
				+ URLEncoder.encode(name) + "&dob=" + URLEncoder.encode(dob)
	...
	protected String getUrlListTopArtist(int offset) {
		checkSelectServer();
		if (offset >= 0) {
			return URL_API_ROOT + "/artists/top?offset=" + offset + "&limit="
					+ DEFAULT_LIMIT;
		} else {
			return URL_API_ROOT + "/artists/top?offset=0&limit="
					+ DEFAULT_LIMIT;
		}
	}
	...
	protected String getSubscribedFriends(int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/account/myidol?offset=" + offset + "&limit="
				+ DEFAULT_LIMIT;
	}
	...
	protected String searchSubcribedFriends(int offset, String keyword) {
		checkSelectServer();
		if (offset >= 0) {
			return URL_API_ROOT + "/account/search?type=idol&keyword="
					+ URLEncoder.encode(keyword) + "&offset=" + offset + "&limit=" + DEFAULT_LIMIT;
		} else {
			return URL_API_ROOT + "/account/search?type=idol&keyword="
					+ URLEncoder.encode(keyword) + "&offset=" + offset + "&limit=" + DEFAULT_LIMIT;
		}
	...
	protected String findUser(String keyword, int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/friends/search?keyword="
				+ URLEncoder.encode(keyword) + "&offset=" + offset + "&limit="
				+ DEFAULT_LIMIT;
	}
	...
	protected String getSubscribedMeFriends(int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/account/myfan?offset=" + offset + "&limit="
				+ DEFAULT_LIMIT;
	}
	...
	protected String searchSubcribedMeFriends(int offset, String keyword) {
		checkSelectServer();
		if (offset >= 0) {
			return URL_API_ROOT + "/account/search?type=fan&keyword=" + keyword
					+ "&limit=" + DEFAULT_LIMIT;
		} else {
			return URL_API_ROOT + "/account/search?type=fan&keyword=" + keyword
					+ "&limit=" + DEFAULT_LIMIT;
	...
	public String getUrlChangePassword(String username, String oldpass,
			String newpass) {
		checkSelectServer();
		return URL_API_ROOT + "/account/changepass?username=" + username
	...
	protected String getUrlListAlbumByArtist(int artistId, int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/artists/" + artistId + "/albums?offset="
	...
	protected String getUrlListNewAlbum(int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/albums/featured?offset=" + offset + "&limit="
				+ DEFAULT_LIMIT;
	}
	...
	protected String GetUrlUnregisterFriendOnFB(String access_token, int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/friends/search/fb/unregistered?access_token="
				+ access_token + "&offset=" + offset + "&limit="
				+ DEFAULT_LIMIT;
	}
	...
	protected String getUrlFriendOnGmail(String access_token) {
		checkSelectServer();
		return URL_API_ROOT + "/friends/search/gmail?access_token="
				+ access_token;
	}
	...
	protected String getUrlListTopSong(int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/songs/featured?offset=" + offset + "&limit="
				+ DEFAULT_LIMIT;
	}
	...
	protected String getUrlListSongByArtist(int artistId, int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/artists/" + artistId + "/songs?offset="
				+ offset + "&limit=" + DEFAULT_LIMIT;
	}
	...
	protected String getUrlDownloadBySongId(int songId, String audioProfileId) {
		checkSelectServer();
		if (audioProfileId.equals(""))
			return URL_API_ROOT + "/songs/" + songId + "/download";
		else
			return URL_API_ROOT + "/songs/" + songId + "/download?profileid="
					+ audioProfileId;
	}
	...
	protected String getUrlListenBySongId(int songId, String audioProfileId) {
		checkSelectServer();
		if (audioProfileId.equals(""))
			return URL_API_ROOT + "/songs/" + songId + "/listen";
		else
			return URL_API_ROOT + "/songs/" + songId + "/listen?profileid="
					+ audioProfileId;
	}
	...
	protected String getUrlSearchSong(String keyword, int offset) {
		...
		return URL_API_ROOT + "/songs/?keyword=" + keyword + "&offset="
				+ offset + "&limit=" + DEFAULT_LIMIT;
	}
	...
	protected String getUrlSearchAlbum(String keyword, int offset) {
		checkSelectServer();
		keyword = URLEncoder.encode(keyword);
		return URL_API_ROOT + "/albums/?keyword=" + keyword + "&offset="
				+ offset + "&limit=" + DEFAULT_LIMIT;
	}
	...
	protected String getUrlSearchArtist(String keyword, int offset) {
		checkSelectServer();
		keyword = URLEncoder.encode(keyword);
		return URL_API_ROOT + "/artists?keyword=" + keyword + "&offset="
				+ offset + "&limit=" + DEFAULT_LIMIT;
	}
	...
	protected String getUrlSearchPlaylist(String keyword, int offset) {
		checkSelectServer();
		keyword = URLEncoder.encode(keyword);
		return URL_API_ROOT + "/playlist?keyword=" + keyword + "&offset="
				+ offset + "&limit=" + DEFAULT_LIMIT;
	}
	...
	protected String getUrlSearchVideo(String keyword, int offset) {
		checkSelectServer();
		keyword = URLEncoder.encode(keyword);
		return URL_API_ROOT + "/videos/?keyword=" + keyword + "&offset="
				+ offset + "&limit=" + DEFAULT_LIMIT;
	}
	...
	protected String getUrlListTopPlaylist(int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/playlist/features?offset=" + offset + "&limit="
				+ DEFAULT_LIMIT;
	}
	...
	protected String getUrlListTopVideo(int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/videos/featured?offset=" + offset + "&limit="
				+ DEFAULT_LIMIT;
	}

	protected String getUrlListVideoByArtist(int artistId, int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/artists/" + artistId + "/videos?offset="
				+ offset + "&limit=" + DEFAULT_LIMIT;
	}
	...
	protected String getUrlListTopNews(int offset) {
		checkSelectServer();
		return URL_API_ROOT + "/news?offset=" + offset + "&limit="
				+ DEFAULT_LIMIT;
	}
	 ...
	 * /songs/audioprofile
	 */
	protected String getUrlListAudioProfile() {
		checkSelectServer();
		return URL_API_ROOT + "/songs/audioprofile";
	}
