...
public class FeatureHomeActivity extends AbstractSongAcitivity implements
		Const, AutoLoadingListener {
	private static final String LOG_TAG = Const.LOG_PREF
			+ FeatureHomeActivity.class.getSimpleName();
	private static final int[] icons = new int[] {
			R.drawable.ic_listview_music, R.drawable.ic_listview_video,
			R.drawable.ic_listview_album, R.drawable.ic_listview_playlist,
			R.drawable.ic_listview_news, R.drawable.ic_listview_billboard };

	private static final int[] titles = new int[] {
			R.string.feature_song_title, R.string.feature_video_title,
			R.string.feature_album_title, R.string.feature_playlist_title,
			R.string.feature_news_title, R.string.feature_billboard_title };
	...
	private boolean reloadList = false, offlineMode = false,
	...
	protected void onPause() {
		super.onPause();
		/*
		 * if (songListAdapter != null) { songListAdapter.onPause();
		 * songListAdapter.notifyDataSetChanged(); }
		 */
	}
	...
	OnScrollListener onFeatureScrollListener = new OnScrollListener() {
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			if (scrollState == 0
					...
					&& scrollable
					&& networkConnected
					&& featureListView.getAdapter().getCount() < LIST_LIMITED - 20
	...
	OnItemClickListener onFeatureHomeItemClick = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> adapter, View view,
			...
			if (offlineMode) {
				...
				if (networkConnected) {
					// truongvv
					findViewById(R.id.view_feature_list1).setVisibility(
							View.VISIBLE);
					...
					case R.drawable.ic_listview_music:
						showNext();
						if (songListAdapter != null
								&& songListAdapter.getCount() > 0) {
							setAdapter(songListAdapter);

						} else if (songListAdapter != null
								&& songListAdapter.getCount() == 0) {
							songListAdapter = null;
							setAdapter(null);
							new FeatureLoadTask().execute(0);
					...
					case R.drawable.ic_listview_billboard:
						Intent billboardIntent = new Intent(getParent(),
								BillboardInfoActivity.class);
						musicDiscoveryStack.startChildActivity(
								BillboardInfoActivity.class.getName(),
								billboardIntent);
					}
				} else {
			...
			if (offlineMode) {
				...
				if (NetworkUtility.hasNetworkConnection()) {
					...
					if (obj instanceof Artist) {
						...
						b = new Bundle();
						b.putInt(Const.BUNDLE_STACK_TYPE,
								Const.TYPE_FEATURE_STACK);
						Session.putSharedObject(b, FeatureHomeActivity.this,
								artist);
						musicDiscoveryStack.startChildActivity(
								ArtistInfoActivity.class.getName(), new Intent(
										getParent(), ArtistInfoActivity.class)
										.putExtras(b));
						...
						b.putInt(Const.BUNDLE_STACK_TYPE,
								Const.TYPE_FEATURE_STACK);
						musicDiscoveryStack.startChildActivity(
								AlbumInfoActivity.class.getName(), new Intent(
										getParent(), AlbumInfoActivity.class)
										.putExtras(b));
						...
						NowPlayingList
								.init(NowPlayingList.TYPE_NORMAL,
										position
												- featureListView
														.getHeaderViewsCount(),
	...
	private void openVideo(Video video) {
		...
		if (!jsonWatchUrl.isSuccess()) {
			...
			try {
				...
				// musicDiscoveryStack.startChildActivity("Video_player",
				// intent);
			} catch (Throwable e) {
	...
	private void setListAdapter(final List<Object> objs, int type) {
		switch (featureType) {

		case R.drawable.ic_listview_music:
			if (songListAdapter == null) {
				songListAdapter = new SongListAdapter(FeatureHomeActivity.this,
						objs, false, FeatureHomeActivity.class.getName());
				setAdapter(songListAdapter);
			} else {
				songListAdapter.notifyListObjectChanged(objs, type);
		...
		case R.drawable.ic_listview_video:
			if (videoAdapter == null) {
				videoAdapter = new FeaturedListAdapter(
						FeatureHomeActivity.this, objs, false);
				setAdapter(videoAdapter);
			} else {
				videoAdapter.notifyListObjectChanged(objs, type);
		...
		case R.drawable.ic_listview_album:
			if (albumAdapter == null) {
				albumAdapter = new FeaturedListAdapter(
						FeatureHomeActivity.this, objs, false);
				setAdapter(albumAdapter);
			} else {
				albumAdapter.notifyListObjectChanged(objs, type);
		...
		case R.drawable.ic_listview_playlist:
			if (playlistAdapter == null) {
				playlistAdapter = new FeaturePlaylistAdapter(
						FeatureHomeActivity.this, objs, false);
				setAdapter(playlistAdapter);
			} else {
				playlistAdapter.notifyListObjectChanged(objs, type);
		...
		case R.drawable.ic_listview_news:
			if (newsAdapter == null) {
				newsAdapter = new FeaturedListAdapter(FeatureHomeActivity.this,
						objs, false);
				setAdapter(newsAdapter);
			} else {
				newsAdapter.notifyListObjectChanged(objs, type);
	...
	private void showPrevious() {
		...
		if (getMode() == MODE_NEWS) {
			...
			backBtn.setVisibility(Button.GONE);
			findViewById(R.id.view_feature_list1).setVisibility(View.GONE);
		}
	}
		...
		public View getView(final int position, View convertView,
			...
			if (convertView == null) {
				convertView = getLayoutInflater().inflate(
						R.layout.view_listview_row_feature_home, null);
				holder = new ViewHolder();
				holder.icon = (ImageView) convertView
						.findViewById(R.id.feature_home_item_icon);
				holder.title = (TextView) convertView
						.findViewById(R.id.feature_home_item_title);
	...
	class FeatureLoadTask extends AsyncTask<Integer, Integer, List<Object>> {
			...
			try {
				...
				case R.drawable.ic_listview_music: {
					if (songListAdapter == null || reloadList) {
						...
						JsonSong js = JsonSong.loadTopSongList(songListAdapter
								.getCount());
						if (js.isSuccess()) {
							objs.addAll(js.songs);
							totalFound = js.totalFound;
				...
				case R.drawable.ic_listview_video: {
					if (videoAdapter == null || reloadList) {
						...
						JsonVideo jav = JsonVideo.loadTopVideoList(videoAdapter
								.getCount());
						if (jav.isSuccess()) {
							objs.addAll(jav.videos);
							totalFound = jav.totalFound;
				...
				case R.drawable.ic_listview_album:
					if (albumAdapter == null || reloadList) {
						...
						JsonAlbum jal = JsonAlbum
								.loadNewAlbummList(albumAdapter.getCount());
						if (jal.isSuccess()) {
							objs.addAll(jal.albums);
							totalFound = jal.totalFound;
						}
				...
				case R.drawable.ic_listview_playlist:
					if (playlistAdapter == null || reloadList) {
						JsonPlaylist jap = JsonPlaylist
								.loadTopPlaylistList(params[0]);
						if (jap.isSuccess()) {
						...
						JsonPlaylist jap = JsonPlaylist
								.loadTopPlaylistList(playlistAdapter.getCount());
						if (jap.isSuccess()) {
							objs.addAll(jap.playlists);
							totalFound = jap.totalFound;
						}
				...
				case R.drawable.ic_listview_news:
					if (newsAdapter == null || reloadList) {
						...
						JsonNews jsn = JsonNews.loadNewsList(newsAdapter
								.getCount());
						if (jsn.isSuccess()) {
							objs.addAll(jsn.newsEntries);
						}
		...
		protected void onPostExecute(List<Object> result) {
			...
			if (reloadList) {
				setListAdapter(result, TYPE_LIST_REPLACED);
				reloadList = false;
			} else {
				setListAdapter(result, TYPE_LIST_ADDED);
			}

			Time now = new Time();
			now.setToNow();
			String updated = getString(R.string.pull_to_refresh_last_updated)
					+ ": " + now.format("%H:%M %d-%m-%Y");
			featureListView.onRefreshComplete(updated);

			// TruongVV
			// When back
			if (homeListView.getVisibility() == View.VISIBLE) {
				// featureListView.setVisibility(View.GONE);
			}
		}
	}
	...
	private void saveMode(int mode) {
		SharedPreferences sharedPreferences = getSharedPreferences(
				FeatureHomeActivity.class.getSimpleName(), 0);
	...
	private int getMode() {
		SharedPreferences sharedPreferences = getSharedPreferences(
				FeatureHomeActivity.class.getSimpleName(), 0);
		return sharedPreferences.getInt("mode", MODE_NORMAL);
	}
}
