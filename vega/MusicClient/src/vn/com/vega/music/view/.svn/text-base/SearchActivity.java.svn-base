...
public class SearchActivity extends AbstractSongAcitivity implements Const {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		View emptyView = ((LayoutInflater) getSystemService(LAYOUT_INFLATER_SERVICE))
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		if (songListAdapter != null
				&& tabs.getCheckedRadioButtonId() == R.id.search_tab_songs) {
	...
	OnScrollListener onListScrollListener = new OnScrollListener() {
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			switch (tabs.getCheckedRadioButtonId()) {
			case R.id.search_tab_songs:
				if (scrollState == 0 && endListReacher
						&& (songFlag == SCROLLABLE_VALUE)
						&& !dataStore.isInOfflineMode()
						&& NetworkUtility.hasNetworkConnection()
						&& listView.getAdapter().getCount() < LIST_LIMITED - 20) {
					new SearchTask().execute(0);
				}
				break;
			case R.id.search_tab_album:
	...
	OnItemClickListener onItemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> adapter, View view,
			...
			if (obj instanceof Song) {
				...
				b = new Bundle();
				b.putInt(Const.BUNDLE_STACK_TYPE, Const.TYPE_SEARCH_STACK);
				Session.putSharedObject(b, SearchActivity.this, album);

				searchTabHolder.startChildActivity(AlbumInfoActivity.class
						.getName(), new Intent(SearchActivity.this,
				...
				searchTabHolder.startChildActivity(ArtistInfoActivity.class
						.getName(), new Intent(SearchActivity.this,
				...
				searchTabHolder.startChildActivity(
						MyPlaylistDetailActivity.class.getName(), new Intent(
								SearchActivity.this,
	...
	private void setListAdapter(final List<Object> objects) {
		...
		case R.id.search_tab_songs:
			if (resultFound > 0) {
				if (songListAdapter == null) {
					songFlag = SCROLLABLE_VALUE;
					songListAdapter = new SongListAdapter(SearchActivity.this,
							objects, false, SearchActivity.class.getName());
					listView.setAdapter(songListAdapter);
		...
		case R.id.search_tab_album:
			if (resultFound > 0) {
				if (albumAdapter == null) {
					albumFlag = SCROLLABLE_VALUE;
					albumAdapter = new FeaturedListAdapter(SearchActivity.this,
							objects, false);
					listView.setAdapter(albumAdapter);
				} else {
			...
			if (resultFound > 0) {
				if (playlistAdapter == null) {
					...
					playlistAdapter = new FeaturePlaylistAdapter(
							SearchActivity.this, objects, false);
					listView.setAdapter(playlistAdapter);
				} else {
		...
		case R.id.search_tab_videos:
			if (resultFound > 0) {
				if (videoAdapter == null) {
					videoFlag = SCROLLABLE_VALUE;
					videoAdapter = new FeaturedListAdapter(SearchActivity.this,
							objects, false);
					listView.setAdapter(videoAdapter);
				} else {
		...
		case R.id.search_tab_artist:
			if (resultFound > 0) {
				if (artistAdapter == null) {
					artistFlag = SCROLLABLE_VALUE;
					artistAdapter = new FeaturedListAdapter(
							SearchActivity.this, objects, false);
					listView.setAdapter(artistAdapter);
				} else {
		...
		protected List<Object> doInBackground(Integer... args) {
			...
			case R.id.search_tab_songs:
				if (songListAdapter == null) {
					...
					JsonSong jos = JsonSong.search(keySearch,
			...
			case R.id.search_tab_album:
				if (albumAdapter == null) {
					...
					JsonAlbum jal = JsonAlbum.search(keySearch,
							albumAdapter.getCount());
					if (jal.isSuccess()) {
						objs.addAll(jal.albums);
						resultFound = jal.totalFound;
						// chacha's api do not return total result
			...
			case R.id.search_tab_playlist:
				if (playlistAdapter == null) {
					...
					JsonPlaylist jpl = JsonPlaylist.search(keySearch,
							playlistAdapter.getCount());
					if (jpl.isSuccess()) {
						objs.addAll(jpl.playlists);
						resultFound = jpl.totalFound;
						// chacha's api do not return total result
			...
			case R.id.search_tab_artist:
				if (artistAdapter == null) {
					...
					JsonArtist jar = JsonArtist.search(keySearch,
							artistAdapter.getCount());
					if (jar.isSuccess()) {
						objs.addAll(jar.artists);
						resultFound = jar.totalFound;
						// chacha's api do not return total result
			...
			case R.id.search_tab_videos:
				if (videoAdapter == null) {
					...
					JsonVideo jpl = JsonVideo.search(keySearch,
							videoAdapter.getCount());
					if (jpl.isSuccess()) {
						objs.addAll(jpl.videos);
						resultFound = jpl.totalFound;
						// chacha's api do not return total result
