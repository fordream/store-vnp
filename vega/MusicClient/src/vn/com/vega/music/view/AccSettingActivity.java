...
public class AccSettingActivity extends Activity {
	...
	private RelativeLayout userInfoLayout, accInfoLayout, accConnectingLayout,
			nofifyConfigLayout, unsubcribeLayout, changePassLayout;
	...
	protected static final int ACC_INFO_SUCCESS = 4;
	protected static final int ACC_INFO_FAILED = 5;
	...
	private void initView() {
		...
		accInfoLayout = (RelativeLayout) findViewById(R.id.account_info_layout);
		accInfoLayout.setOnClickListener(onAccInfoLayoutListener);
	}
	...
	OnClickListener onChangePassLayoutListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			newPassConfirmEdt = (EditText) dialog
					.findViewById(R.id.new_pass_edt_2);
	...
	private void changePassword() {
		...
		Thread dataInitializationThread = new Thread() {
			public void run() {
				JsonAuth jau = JsonAuth.changePassword(dataStore.getMsisdn(),
						oldPassEdt.getText().toString(), newPassEdt.getText()
								.toString());
				if (jau.isSuccess()) {
					dataStore.setPassword(newPassEdt.getText().toString());
		...
		v.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case R.id.change_pass_save_btn:
					if (!oldPassEdt.getText().toString().trim()
							.equals(dataStore.getSavedPassword())) {
						Toast.makeText(mContext, "Mật khẩu cũ không đúng",
								Toast.LENGTH_SHORT).show();
					} else if (!newPassEdt.getText().toString()
							.equals(newPassConfirmEdt.getText().toString())) {
						Toast.makeText(mContext, "Mật khẩu mới không khớp",
								Toast.LENGTH_SHORT).show();
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case CHANGE_PASS_FAILED:
				Toast.makeText(mContext, "Đổi mật khẩu không thành công",
						Toast.LENGTH_SHORT).show();
				break;
			case CHANGE_PASS_SUCCESS:
				if (dialog != null)
					dialog.dismiss();
				Toast.makeText(mContext, "Đổi mật khẩu thành công",
						Toast.LENGTH_SHORT).show();
				break;
			case ACC_INFO_FAILED:
				Toast.makeText(mContext, "Không lấy được thông tin tài khoản",
						Toast.LENGTH_SHORT).show();
				break;
			case ACC_INFO_SUCCESS:
				if (!unsubscribe.packageCode.equals("")) {
					String str = "Bạn đang sử dụng gói cước "
							+ unsubscribe.packageCode
							+ ". Thời gian sử dụng từ "
							+ unsubscribe.beginDate + " đến "
							+ unsubscribe.endDate;
					showAlert(str);
				}
	...
	OnClickListener onAccInfoLayoutListener = new OnClickListener() {

		@Override
		public void onClick(View arg0) {
			// TODO Auto-generated method stub
			mProgressDialog = new ProgressDialog(mContext);
			mProgressDialog.setMessage("Đang lấy thông tin...");
			mProgressDialog.setCancelable(false);
			mProgressDialog.show();
			new Thread(new Runnable() {
				@Override
				public void run() {
					unsubscribe = JsonAuth.identify();
					if (!unsubscribe.isSuccess()) {
						mHandler.sendEmptyMessage(ACC_INFO_FAILED);
					} else {
						mHandler.sendEmptyMessage(ACC_INFO_SUCCESS);
					}
				}
			}).start();
	...
	OnClickListener onUserInfoLayoutListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			// "Tính năng đang trong quá trình cập nhật",
			// Toast.LENGTH_SHORT).show();
		}
	};
