...
public class WelcomeActivity extends Activity implements Runnable {
	...
	public static WelcomeActivity mLock;
	...
	private void gotoOfflineMode(String msg) {
		...
		builder.setMessage(msg)
				.setCancelable(false)
				.setPositiveButton(mContext.getString(R.string.confirm_yes),
				...
				.setNegativeButton(mContext.getString(R.string.confirm_no),
						new DialogInterface.OnClickListener() {
							...
							// No , i don't wanna go to offline mode ------>
							// finish app
							public void onClick(DialogInterface dialog, int id) {
								try {
									DataStore dataStore = DataStore
	...
	private void preparingRegister(boolean wantRegister) {
		try {
			// for sure
			...
			if (wantRegister)
				...
				bundle.putBoolean(Const.KEY_NOT_REGISTERED, false);
	...
	private void confirmRegister(String msg) {
		AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
		builder.setMessage(msg)
				.setCancelable(false)
				.setPositiveButton(mContext.getString(R.string.confirm_yes),
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case ASK_OFFLINE:
				gotoOfflineMode(mContext.getString(R.string.setting_question_switch_to_offline));
				break;
			case NO_INTERNET:
				// show message then exit app
				showAlert(mContext.getString(R.string.no_network_connection));
				break;
			case SERVER_ERROR:
				// showAlert(getString(R.string.login_screen_error_auto_login_unsuccessfully));
				gotoOfflineMode(mContext.getString(R.string.server_error_go_to_offline));
				break;
			case NOT_AUTHENTICATED:
				// show message then exit app
				if (authenMessage.trim().equals("")) {
					// showAlert(getString(R.string.login_screen_error_authenticate_unsuccessfully));
					gotoOfflineMode(mContext.getString(R.string.not_authenticated_go_to_offline));
				} else {
			...
			case NOT_REGISTERED_MSG:
				confirmRegister("Bạn chưa đăng ký dịch vụ. Bạn có muốn đăng ký không ?");
				break;
			case SHOW_FIRST_PROGRESS:
				if (!threadModeEnabled && mProgressBar != null)
			...
			case SUCCESS:
				if (!threadModeEnabled) {
					Intent intent1 = new Intent(mContext, TabHolder.class)
							.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
	...
	public void run() {
		synchronized (WelcomeActivity.class) {
			if (threadModeEnabled && this != mLock) {
				return;
			}
			mIsRunning = true;
		}
						...
						// ok, network is nice
							...
							if (identify.isSuccess()) {
								...
								if (identify.packageCode.equalsIgnoreCase("")) {
									...
									dataStore
											.updateAccountConfig(identify.hashAccount);
				...
				if (authenticated == AUTHEN_SUCCESS) {
					...
					// okie, network is good
						...
						if (identify.isSuccess()) {
							...
							// No, not registered
							if (identify.packageCode.trim()
									.equalsIgnoreCase("")) {
								// register flow
								registerMessage = identify.getErrorMessage();
								dataStore.setConfig(
										Const.KEY_TEMP_PHONE_NUMBER,
							...
							Bundle bundle = new Bundle();
							bundle.putBoolean(
									LoginActivity.NEED_AUTHENTICATION, false);
							Intent intent = new Intent(mContext,
									StartPageActivity.class).setFlags(
