...
public class MyPlaylistAdapter extends BaseAdapter implements
		DownloadStatusListener, OnScrollListener {

	public static final String LOG_TAG = Const.LOG_PREF
			+ MyPlaylistAdapter.class.getSimpleName();
	...
	public MyPlaylistAdapter(Context context, DataStore store,
			boolean _selection, List<Object> list) {
		mContext = context;
	...
	protected void getSectionHeaderPosition(List<Object> objs) {
		...
		else if (firstSectionHeaderPos != ERROR_HEADER_INDEX)
			secondSectionHeaderPos = firstSectionHeaderPos + user_created_count
					+ 1; // need to replace
		else if (flag)
			...
			if (obj instanceof Song) {
				...
				if (song.isWaitToDownload()) {
					if ((dldSong != null) && (song.id == dldSong.id)) {
						mHashProgress
								.put(song.id, DownloadManager
										.getCurrentDownloadSongPercent());
					}
					// else {
				...
				if ((dldPlaylist != null) && (playlist.id == dldPlaylist.id)) {
					mHashProgress
							.put(playlist.id, DownloadManager
									.getCurrentDownloadPlaylistPercent());
				}
			}
		}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (object instanceof Song) {
			...
			RelativeLayout firstRegion = (RelativeLayout) convertView
					.findViewById(R.id.my_playlist_row_region_first);
			...
			if (needSectionHeader) {
				if (position == 0)
					...
					convertView
							.setBackgroundResource(R.drawable.bg_section_header_ex);
				} else if (position == secondSectionHeaderPos)
					convertView
							.setBackgroundResource(R.drawable.bg_section_header);
				else if (position == (getCount() - 1))
					convertView
							.setBackgroundResource(R.drawable.selector_listview_last);
				else
	...
	public void onPlaylistDownloadProgress(Playlist pl, int totalSong,
		...
		if (!isScrolling) {
			...
			if (cached instanceof Playlist) {
				...
				if (_position >= firstVisibleItem
						&& _position <= lastVisibleItem) {
					notifyDataSetChanged();
				}
			}
		}
	...
	public void onDownloadInOfflineMode() {
		...
		Toast.makeText(mContext, "Đang ở chế độ offline", Toast.LENGTH_SHORT)
				.show();
	}
	...
	public void onDownloadVia3gDisable() {
		// TODO Auto-generated method stub
		Toast.makeText(mContext, "Không cho phép tải nhạc qua 3G",
