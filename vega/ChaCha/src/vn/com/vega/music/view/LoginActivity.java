...
public class LoginActivity extends Activity {
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case LOGIN_SUCCESS:
				// open Music page
				Intent intent = new Intent(getApplicationContext(), TabHolder.class);
	...
	private void loginAgain() {
		...
		mProgressDialog.setMessage(getString(R.string.login_screen_login_in_progress));
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (bundle != null) {
			...
			oldUsername = (bundle.getString(Const.KEY_MSISDN) != null ? bundle.getString(Const.KEY_MSISDN) : "");
			oldPassword = (bundle.getString(Const.KEY_PASSWORD) != null ? bundle.getString(Const.KEY_PASSWORD) : "");
		...
		footerView = LayoutInflater.from(LoginActivity.this).inflate(R.layout.view_listview_normal_footer, null);
	...
	public void doGetPass(View view) {
		...
		if (!SMS_TO.trim().equals("") && !SMS_BODY.trim().equals("")) {
			...
			builder.setTitle("Quên mật khẩu").setMessage("Bạn có muốn gửi tin nhắn với nội dung " + SMS_BODY + " tới tổng đài " + SMS_TO + " để lấy lại mật khẩu không ?").setCancelable(false)
					.setPositiveButton(mContext.getString(R.string.confirm_yes), new DialogInterface.OnClickListener() {
	...
	public void doRegister(View view) {
		...
		mProgressDialog.setMessage(getString(R.string.login_screen_load_package));
		mProgressDialog.setCancelable(false);
		mProgressDialog.show();
	...
	private void showSMSDialog() {
		...
		Button cancelBtn = (Button) subDialog.findViewById(R.id.sms_dialog_cancel_btn);
		...
		okBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				subDialog.dismiss();
				mProgressDialog = new ProgressDialog(mContext);
				mProgressDialog.setMessage(getString(R.string.login_screen_registering));
				mProgressDialog.setCancelable(false);
				mProgressDialog.show();
				new Thread(new Runnable() {
					@Override
					public void run() {
						JsonAuth jsonAuth = JsonAuth.subscribe(selectedPackage.code);
						if (!jsonAuth.isSuccess()) {
							// failed
							// mHandler.sendEmptyMessage(SUBSCRIBE_FAILED);
							sendSMS(selectedPackage.smsServer, selectedPackage.smsContent);
							mHandler.sendEmptyMessage(SEND_SMS);
						} else {
							// success
							DataStore dataStore = DataStore.getInstance();
							if (jsonAuth.username != null)
								dataStore.setMsisdn(jsonAuth.username);
		...
		TextView packageDes = (TextView) subDialog.findViewById(R.id.sms_first_txt);
		TextView smsContent = (TextView) subDialog.findViewById(R.id.sms_second_txt);

		packageDes.setText(selectedPackage.description);
		smsContent.setText("Giá cước dịch vụ là " + selectedPackage.price + " .Bạn có muốn đăng ký dịch vụ không ?");
		subDialog.show();
	}
	...
	public void showPackageDialog() {
		...
		ListView _listview = (ListView) dialog.findViewById(R.id.common_context_menu_listview);
		Button _closeBtn = (Button) dialog.findViewById(R.id.common_context_menu_close_btn);
		...
		TextView _title = (TextView) dialog.findViewById(R.id.common_context_menu_title_txt);
	...
	private class DialogAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			ViewHolder holder;
			if (convertView == null) {
				convertView = mInflater.inflate(R.layout.view_listview_row_context_menu, null);
				holder = new ViewHolder();
				holder.image = (ImageView) convertView.findViewById(R.id.common_context_menu_row_thumbnail_img);
				holder.arrow = (ImageView) convertView.findViewById(R.id.common_context_menu_row_arrow_img);
				holder.first_text = (TextView) convertView.findViewById(R.id.common_context_menu_row_first_txt);
				holder.second_text = (TextView) convertView.findViewById(R.id.common_context_menu_row_second_txt);
