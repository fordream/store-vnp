...
public class UserInfoActivity extends Activity implements Const {
	...
	public void updateUserInfo(View v) {
		...
		final String email = mEmailEditable ? edtEmail.getText().toString()
				: null;
		final String dob = String.format("%04d-%02d-%02d", mYear, mMonth, mDay);
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				JsonAuth update = JsonAuth.updateAccount(name, phone, email,
						dob, gender, address);
				if (update.isSuccess()) {
	...
	private void initView() {
		...
		updateDisplay(); // display the current date

		// gender
		edtGender.setOnClickListener(onGenderListener);
		edtGender.setText(GenderToString(Integer.valueOf(
				dataStore.getConfig(KEY_GENDER)).intValue()));
	...
	private void updateDisplay() {
		edtBirth.setText(new StringBuilder()
				...
				.append(FormatTime(mMonth + 1)).append("/").append(mYear));
	}
