...
public class JsonAuth extends JsonBase implements Const {
	...
	public static JsonAuth validate(String randomKey) {
		...
		if (result.isSuccess()) {
			try {
				...
				result.mService.sms_content = result.getString(data, "recover_sms_server", false);
				result.mService.sms_server = result.getString(data, "recover_sms_content", false);
	...
	public static JsonAuth uploadAvatar(Bitmap avatar) {
		...
		JsonAuth result = new JsonAuth();
		JSONObject json = result.postAndParseBasicJson(url, avatar);
		if (result.isSuccess()) {
			// what the hell here ???
		}
		return result;
	...
	public static JsonAuth subscribe(String packageID) {
		String url = ClientServer.getInstance().getUrlAuthSubscribe(packageID);
		JsonAuth result = new JsonAuth();
		JSONObject json = result.requestAndParseBasicJson(url);
		result.parseAccount(json);
		return result;
