...
public class OfferFirstAsyn extends AsyncTask<String, String, String> {
	...
	public OfferFirstAsyn(MaisprecoScreen maisprecoScreen, Product product,
			int page, boolean isRelogin, OffersViewControl offersViewControl) {
		super();
	...
	protected String doInBackground(String... params) {
		...
		while (check) {
			...
			offersViewControl.addList(offersResponse2.getLoOffersResponses());
			if (page == 1) {
				offersViewControl.post(new Runnable() {
					public void run() {
						dialog.dismiss();
						offersViewControl.notifyDataSetChangedNearyBy();
					}
				});
			}
			page++;
		...
		if (result != null) {
			...
			builder.setPositiveButton("YES", new OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					new OfferFirstAsyn(maisprecoScreen, product, page, false,
							offersViewControl).execute("");
				}
			});
			builder.show();
		}
