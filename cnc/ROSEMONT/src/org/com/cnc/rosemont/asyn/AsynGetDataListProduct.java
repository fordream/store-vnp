...
public class AsynGetDataListProduct extends CommonAsyncTask {
	private HomeActivity activity;
	boolean flagNetwork = false;
	boolean checkNetwork=true;
	public AsynGetDataListProduct(HomeActivity activity) {
		super(activity);
	...
	protected void onPostExecute(String result) {
		  ...
		  if (flagNetwork) {
		   ...
		   ((HomeActivity) getActivity()).startActivityForResult(intent,CommonApp.REQUESTCODE_FINISH);
...
/*	}public boolean isConnected() {
	URL url;
	  try {
	   ...
	   Log.d("Teburu", "Teburu_receiver Internet available : "
	...
	public boolean isConnected() {
		...
		if ((cm.getNetworkInfo(ConnectivityManager.TYPE_WIFI).getState() == NetworkInfo.State.DISCONNECTING)&&((cm.getNetworkInfo(ConnectivityManager.TYPE_MOBILE).getState() == NetworkInfo.State.CONNECTED))){
			return false;
		}
