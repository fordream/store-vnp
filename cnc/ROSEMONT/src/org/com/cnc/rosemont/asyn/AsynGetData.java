...
public class AsynGetData extends CommonAsyncTask {
	private boolean flagNetwork = false;

	public AsynGetData(SplashActivity activity) {
		super(activity);
	}
	...
	protected String doInBackground(String... arg0) {
		...
		for (int i = 0; i < tempRosemontTable.sizeOfRow(); i++) {
			...
			File dir = new File(CommonApp.PATH + "/" + name);
			Log.i("path..................", ""+CommonApp.PATH + "/" + name);
			if (isConnected()) {
				if (!Common.isNullOrBlank(name) && !dir.exists()) {
	...
	protected void onPostExecute(String result) {
		...
		if (flagNetwork) {
			...
			((SplashActivity) getActivity()).startActivityForResult(intent,
					Common.REQUESTCODE_02);
		} else {
			((SplashActivity) getActivity()).goToCustomTab();
