...
public class AsynGetDataSearch extends CommonAsyncTask {
	private boolean flagNetwork = false;

	public AsynGetDataSearch(Activity activity) {
		super(activity);
	}
	...
	protected void onPostExecute(String result) {
		showDialog(false, "", "");
		if (flagNetwork) {
			DataStore.getInstance().setConfig(Conts.POST, true);
			Intent intent = new Intent(getActivity(),
					DialogWarnningActivity.class);
			((SearchActivity) getActivity()).startActivityForResult(intent,
					CommonApp.REQUESTCODE_FINISH);
		} else {
