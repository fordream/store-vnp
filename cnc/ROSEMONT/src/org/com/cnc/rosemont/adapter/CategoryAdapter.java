...
import org.com.cnc.rosemont.views.ItemCategoriesView;
...
public class CategoryAdapter extends ArrayAdapter<ItemSearch> implements
		SectionIndexer {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (!item.isAnphabe()) {
			convertView = new ItemCategoriesView(getContext());
		...
		if (height >= 0 && convertView instanceof ItemCategoriesView) {
			params = new LayoutParams(LayoutParams.FILL_PARENT, height);
			convertView.setLayoutParams(params);
			((ItemCategoriesView) convertView).updateData(item.getTxtHeader());
