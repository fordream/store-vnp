...
public class ProfileSportDayItemView extends LinearLayout {

	public ProfileSportDayItemView(Context context, AttributeSet attrs) {
		super(context, attrs);
		config(R.layout.profile_sport_day_item, "");
	}

	public ProfileSportDayItemView(Context context) {
		super(context);
		config(R.layout.profile_sport_day_item, "");
	}
	...
	public void setShowHeader() {
		getLinearLayout(R.id.profile_sport_day_item_llheader).setVisibility(
				VISIBLE);
		getLinearLayout(R.id.linearLayout3).setVisibility(VISIBLE);
		getLinearLayout(R.id.LinearLayout15).setVisibility(VISIBLE);
	}
	...
	public void changType(int index, boolean isOld) {
		...
		if (index <= 5 && index >= 1) {
			if (index == 1)
				rerource = R.id.profile_sport_day_item_llheader_cb1;
			else if (index == 2)
				rerource = R.id.profile_sport_day_item_llheader_cb2;
			else if (index == 3)
				rerource = R.id.profile_sport_day_item_llheader_cb3;
			else if (index == 4)
				rerource = R.id.profile_sport_day_item_llheader_cb4;
			else if (index == 5)
				rerource = R.id.profile_sport_day_item_llheader_cb5;
			getCheckBox(rerource).setButtonDrawable(
					isOld ? R.drawable.profile_checkbox1
							: R.drawable.profile_checkbox2);
		}
	}
	...
	public void setChecked(int index, boolean check) {
		if (index == 0)
			getCheckBox(R.id.profile_sport_day_item_llheader_cb1).setChecked(
					check);
		if (index == 1)
			getCheckBox(R.id.profile_sport_day_item_llheader_cb2).setChecked(
					check);
		if (index == 2)
			getCheckBox(R.id.profile_sport_day_item_llheader_cb3).setChecked(
					check);
		if (index == 3)
			getCheckBox(R.id.profile_sport_day_item_llheader_cb4).setChecked(
					check);
		if (index == 4)
			getCheckBox(R.id.profile_sport_day_item_llheader_cb5).setChecked(
					check);
	}
}