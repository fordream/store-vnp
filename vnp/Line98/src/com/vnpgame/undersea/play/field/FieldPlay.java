...
public class FieldPlay implements OnClickListener {
	...
	// private MediaPlayer playerNextLevel;
	...
	public FieldPlay(Play3Screen play3Screen) {
		...
		// /player = MediaPlayer.create(play3Screen, R.raw.click);
		// playerNextLevel = MediaPlayer.create(play3Screen, R.raw.nextlevel);
		// playerlose = MediaPlayer.create(play3Screen, R.raw.lose);
	...
	private class UpdateBoard extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			if (list.size() >= 2) {
				board.remove(list);
				menuFiled.setScore(menuFiled.getScore() + list.size()
						* list.size() * list.size() * 5 * menuFiled.getLevel());
			} else {
				return null;
			}
	...
	private class LoadBoard extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			handler.post(new Runnable() {
				public void run() {
					dialog = ProgressDialog.show(play3Screen, "Load board",
							"Loadding ...");
					board.createNewBoard(menuFiled.getLevel());
				}
			});
