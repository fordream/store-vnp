...
public class C2DMRegistrationReceiver extends BroadcastReceiver {
	...
	public void sendRegistrationIdToServer(String deviceId,String registrationId) {
		String lag =  ISettings.LANGUAGE;
		HttpClient client = new DefaultHttpClient();
		HttpPost post = new HttpPost("http://49.212.161.19/apissff/android-device.php");
		try {
			List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(1);
			// Get the deviceID
			nameValuePairs.add(new BasicNameValuePair("deviceid", deviceId));
			nameValuePairs.add(new BasicNameValuePair("registrationid",registrationId));
			nameValuePairs.add(new BasicNameValuePair("language", lag));
